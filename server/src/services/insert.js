...
export const insertService = async () => {
    try {
        ...
        for (const cate of dataBody) {
            for (const item of cate.body) {
                ...
                if (!labelCodes.some(l => l.code === labelCode)) {
                    labelCodes.push({ code: labelCode, value: item?.header?.class?.classType?.trim() });
                }
                if (!provinceCodes.some(p => p.code === provinceCode)) {
                    provinceCodes.push({ code: provinceCode, value: item?.header?.address?.split(',')?.slice(-1)[0].trim() });
                }
        ...
        await db.Province.insertMany(provinceCodes);
        await db.Label.insertMany(labelCodes);
...
export const createPricesAndAreas = async () => {
    try {
        await Promise.all([
            db.Price.insertMany(dataPrice.map((item, index) => ({
                code: item.code,
                value: item.value,
                order: index + 1
            }))),
            db.Area.insertMany(dataArea.map((item, index) => ({
                code: item.code,
                value: item.value,
                order: index + 1
            })))
